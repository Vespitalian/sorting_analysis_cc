...
think, and solve problems. It can automate complex, 
...
taking on the hard parts of problems.
...
how computation automates logic that humans could perform in theory 
but would take far too long to do efficiently in practice.

### Selection Sort
Selection Sort is one of the most straightforward algorithms and it 
clearly shows how computation can automate human steps. It finds the 
smallest element in a list, moves it to the beginning, and repeats 
the process until everything is in order.

-   Scan the unsorted portion of the list to locate the lowest value.
-   Swap this lowest value with the element at the current starting position of the unsorted sublist.
...
of whether the list is sorted or not. Its comparison count is always 
$O(n^2)$
...
You could follow this same logic manually but when 
looking at larger data this task could take significant 
time and concentration. The computer is able to complete 
this task instantly and without any error.
...
-   Take the next value from the unsorted portion of the list.
...
#### Complexity
This algorithm's performance can vary heavily based on the input. 
If the list is already sorted, it makes one pass, makes no shifts, 
and ends.
...
Like Selection Sort, you could have performed Insertion Sort 
but again, it would be slow and prone to mistakes. Computers 
can do the same operation thousands of times faster.
...
sorted, as it will only make one pass, perform no swaps, and 
exit.
...
-   Divide the unsorted list into two sublists, each half the size of the original.
-   Keep dividing each sublist until every piece has only one element.
-   Merge two sublists together by repeatedly placing the smallest element first.
-   Continue merging until you have combined all sublists back into a fully sorted list.
...
Timsort is the built-in sorting algorithm used by Python. It's what 
under the hood of the sorted() function. It's a hybrid algorithm, 
combining Insertion Sort and Merge Sort. It's designed to be very 
fast on "real-world" data, which often contains partially-sorted 
"runs". It finds these runs, sorts them with Insertion Sort if they 
are small, and then merges them using Merge Sort.
...
rotate_df = df_random.pivot_table(
    ...
    columns="Size",
    values="Time (s)",
    fill_value="-" # Used for bogo sort
)

order = ["Selection Sort","Insertion Sort","Bubble Sort","Merge Sort","Python's Timsort"]
rotate_df = rotate_df.reindex(order, level="Algorithm", fill_value="-")

rotate_df
```
> We use reindex to change the default index values from numerical to using the algorithms as the index.

### Visualization
Scientifc computing allows us to test, visualize, and interpet these
different algorithms instantly. Without automated computation this
would have taken countless hours of manual timing, data collection, 
and graphing.
...
and most of the algorithms are slower than in the random case. By looking at 
these variations in lists, we are able to tell that some algorithms are better 
at different tasks, which is why it's important to know the difference in 
