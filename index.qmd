...
include-in-header:
    - text: | 
        <link rel="icon" type="image/x-icon" href="https://picture.wtf/p/MSIqX8.ico">
...
repetitive, and time-consuming tasks that can take 
hours or even days to complete by hand. This project 
...
- **Thesis**: Scientific computing lets us solve problems that would 
be impossible to solve by hand. By testing sorting 
algorithms in Python, this project shows that computers 
don't just work faster than humans, they help us complete 
goals we couldn't do on our own. Comparing five different 
algorithms across different data distributions, generating 
graphs, and calculating statistics could take weeks of 
manual work. With Python, it takes minutes. This is why 
scientific computing matters.
...
clearly shows how computing can automate human steps. It 
finds the smallest element in a list, moves it to the next position 
in the sorted portion, and repeats the process until everything is 
in order.
...
#### Time Complexity
Selection Sort scans the remaining $n$ elements $n$ times, regardless 
    ...
    for i in range(1,n):
        ...
        for j in range(i-1, -1, -1):
            if lst[j] > current_value:
                ...
                break
        lst[insert_index] = current_value
    return lst
```
> Modified from w3schools.com
...
3. If the value is higher than the next one, swap the values so that the higher value moves right.
...
repeatedly comparing neighbors and swapping them when out of order.
...
## Benchmarking
Having examined the theory behind these algorithms, we will now test
them using Python to verify their expected performance.
...
lengths and data distributions. We measure the time taken for each 
...
n_values = [10, 50, 100, 250, 1000, 2500, 5000]
lsts = ["random_list", "sorted_list", "reversed_list"]
results = []
...
order = ["Selection Sort","Insertion Sort","Bubble Sort","Merge Sort","Timsort"]
rotate_df = rotate_df.reindex(order)
...
> This graph represents the average scenario where the data is completely randomly generated 1 through $n$ (list size).
...
This list reveals each algorithm on its best behavior, that being how well
it can recognize that and confirm that no work is needed. Insertion and 
Bubble sort both perform much better in this scenario because they both 
include logic that checks to see if the list is already sorted. While an
algorithm like Selection Sort goes through the entire list multiple times
verifying that each value is already in its correct spot.

The sorted list shows us that algorithms that adapt to their given data
can outperform rigid algorithms like Merge or Selection Sort. Data is 
rarely completely random, there are patterns in the timestamps of emails,
numbers on credit cards, or even user-generated content. Algorithms that 
take advantage of this fact will perform the best.

We are now going to take a look at our last list type which is a 
...
The reversed list shows the algorithms under maximum stress. Insertion 
Sort has to shift every element all the way to the beginning of the 
list giving us our mathematical maximum for this algorithm. The same 
applies to Bubble Sort where each value has to bubble through 
every position. On the other hand, Selection Sort it remains 
consistent across all variations of the lists.

By looking at these variations in lists, we are able to tell that 
some algorithms are better at different tasks, which is why 
it's important to know the difference in performance for each 
algorithm.

Without these tools all of this would have taken countless 
hours of manual timing, data collection, and graphing. You'd also have 
to look at my hand drawn graphs and no one wants that. Scientific computing 
allows us to test, visualize, and interpret these different algorithms 
instantly. 
...
variance from any system "noise" or other processes. This would make the 
...
complete these tasks instantly and perfectly. This is why scientific 
...
### Bogo Sort
While looking at different sorting algorithms, it doesn't take long
to stumble upon Bogo Sort. Bogo Sort is a joke algorithm used only 
for educational purposes to demonstrate a terrible, inefficient 
approach. Its strategy is simple:
...
- **Worst Case:** It could shuffle the list into the same incorrect orders forever. $\infty$.
...
We will use SymPy to write the equation that tells us
how many possible combinations the lists have when using Bogo 
Sort. This equation will represent the math needed in order 
...
eq = sympy.Eq(P(n), permutations)
print(f"$$ {sympy.latex(eq)} $$")
```

Now let's calculate exactly how many possible combinations
exist for different list sizes. Bogo Sort has to randomly 
stumble upon the **ONE** correct ordering out of all possibilities.
...
Taking a look at the table created above we can see that 
...
This is why we excluded bogo from our benchmarks. Bogo Sort reminds us 
that not all answers to solving a problem are good ones. 
...
output = os.popen('wc -w index.qmd').read()
words = int(output.split()[0])
