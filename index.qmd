...
eq = sympy.latex(sympy.Eq(P(n), permutations))
display(Markdown(f"$$\n{eq}\n$$"))
```
...
We can notice when comparing it to the previous list, Bubble and 
Insertion Sort are significantly better, because of their best
case scenarios where they verify the list is sorted.
